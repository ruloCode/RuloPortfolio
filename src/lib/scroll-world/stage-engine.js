 ...
 *
 * Copia del motor de fauna-scroll-world con dos añadidos para vivir dentro del
 * portfolio (Next + next-intl + Once UI), donde la home comparte layout con el
 * resto de páginas:
 *   - mountStageWorld() devuelve destroy(): quita los listeners globales y
 *     libera los <video>. Sin esto, navegar con <Link> a otra página dejaba al
 *     motor escuchando el scroll ajeno.
 *   - Las reglas que tocan <html> (snap, fondo) van bajo html[data-scroll-world],
 *     atributo que el motor pone al montar y quita al desmontar, para que el
 *     scroll-snap obligatorio no se coma las demás páginas.
 *   - Estado is-past: cuando el scroll pasa la última estación, las capas fijas
 *     (escena, copy, rail) se apagan y el documento sigue en scroll normal con
 *     el contenido que venga debajo del track.
 *
 ...
 *     sections: [{ id, label, still, stillMobile, accent,
 *                  clip, clipMobile,   // string o { av1, hevc, h264 } - ver bestCodec()
 *                  eyebrow, title, body, tags, cta, copyAt }]
 *   })
 */
// ---- seleccion de codec ----
// Cada clip puede venir como string (una URL) o como objeto { av1, hevc, h264 }:
// se elige el mejor codec que el navegador decodifique. AV1 cubre Chrome/Firefox
// (y Safari con hardware M3+); HEVC cubre el resto de Safari/iPhone por hardware;
// H.264 queda como red de seguridad universal.
let CODEC;
function bestCodec() {
  if (!CODEC) {
    const v = document.createElement('video');
    CODEC = v.canPlayType('video/mp4; codecs="av01.0.08M.08"') ? 'av1'
      : v.canPlayType('video/mp4; codecs="hvc1.1.6.L123.B0"') ? 'hevc'
        : 'h264';
  }
  return CODEC;
}
function clipUrl(c) {
  ...
  return c[bestCodec()] || c.h264 || null;
}
...
export function mountStageWorld(container, config) {
  ...
  document.documentElement.setAttribute('data-scroll-world', '');
  ...
  if (config.brand) {
    ...
    const brand = el('a', 'sw-brand'); if (config.brand.href) brand.href = config.brand.href;
  ...
  function ensureVideo(s, eager) {
    ...
    const src = clipUrl((isMobile() && s.cfg.clipMobile) ? s.cfg.clipMobile : s.cfg.clip);
    if (!src) return null;
    const v = document.createElement('video');
    ...
    // El atributo `autoplay` arranca también los clips PRECARGADOS (ocultos), y
    // los decodificadores móviles son un recurso limitado: si empieza a sonar
    // en una sección que no es la activa, se pausa en frame 0 hasta su turno.
    v.addEventListener('playing', () => {
      if (S[active] !== s) { try { v.pause(); v.currentTime = 0; } catch (e) {} }
    });
  ...
  // `immediate` importa en los reintentos por gesto: play() debe ejecutarse
  // dentro de la ventana de activación del usuario o iOS lo vuelve a rechazar.
  function playSection(s, immediate) {
    ...
    const start = () => {
      ...
      if (p && p.catch) {
        p.then(() => { s.playing = true; }).catch(() => {
          ...
          // El siguiente toque en pantalla lo reintenta (listener de click abajo).
          showCopy(indexOf(s));
        });
      }
    };
    // Móvil: el clip espera a que el crossfade aterrice antes de arrancar. La
    // escena entra mostrando su frame 0 estático (el póster ES ese frame), y el
    // vuelo empieza cuando ya se ve — sin esto, la activación a mitad del snap
    // se comía el primer medio segundo del clip detrás del fundido.
    // pause() primero: el atributo `autoplay` (necesario para iOS) hace que el
    // clip precargado ya venga reproduciéndose oculto — sin pausa, el seek a 0
    // no congela nada y el retardo no tendría efecto.
    if (isMobile() && !immediate) {
      try { v.pause(); } catch (e) {}
      s.playTimer = setTimeout(start, 280);
    } else start();
  }

  function stopSection(s) {
    clearTimeout(s.playTimer);   // un arranque diferido no debe disparar tras salir
  ...
  function onScroll() {
    ...
    requestAnimationFrame(() => {
      ...
      activate(i);
      // Pasada la última estación el documento sigue (trayectoria, blog...):
      // se apagan las capas fijas y se pausa el clip, que ya no se ve.
      const past = window.scrollY > (N - 1) * h + h * 0.5;
      if (past !== isPast) {
        isPast = past;
  ...
  function onFirstGesture() {
    ...
    const cur = S[active];
    if (cur && (!cur.video || cur.video.paused)) playSection(cur, true);
  }
  ['pointerdown', 'touchstart', 'wheel', 'keydown'].forEach(ev =>
    window.addEventListener(ev, onFirstGesture, { once: true, passive: true }));

  // Un toque en la pantalla siempre hace algo útil (y a diferencia del priming
  // de arriba, este listener es PERMANENTE — el bug de móvil era que tras el
  // primer gesto ya nada reintentaba un play() rechazado):
  //  - clip actual pausado (autoplay bloqueado: modo de bajo consumo, ajustes
  //    restrictivos) → el tap es el gesto que lo desbloquea y lo arranca;
  //  - clip reproduciéndose o terminado → en móvil el tap avanza a la siguiente
  //    estación: un toque o un gesto de scroll, cualquiera cambia de escena.
  function onTap(e) {
    const t = e.target;
    if (t && t.closest && t.closest('a,button')) return;
    const cur = S[active];
    if (!cur) return;
    if (!reduce && (!cur.video || (cur.video.paused && !cur.video.ended))) {
      playSection(cur, true);
      return;
    }
    if (isMobile()) jumpTo(Math.min(active + 1, N - 1));
  }
  ...
  activate(0);
  function onLoad() { if (active === 0 && !S[0].playing) playSection(S[0]); }
  ...
  function destroy() {
    ...
    S.forEach(s => { clearTimeout(s.playTimer); releaseVideo(s); });
  ...
  function ctaBtns(cta) {
    ...
    let h = '';
    if (cta.primary) h += `<a class="sw-btn sw-btn--primary"${attr(cta.primary)}>${esc(cta.primary.label)}</a>`;
    if (cta.secondary) h += `<a class="sw-btn sw-btn--ghost"${attr(cta.secondary)}>${esc(cta.secondary.label)}</a>`;
    return h;
  }
}
...
function injectStageCSS() {
  ...
  html[data-scroll-world],html[data-scroll-world] body{margin:0;background:var(--sw-bg,#F5EDE0);}
  ...
  html[data-scroll-world]{scroll-snap-type:y mandatory;}
  ...
  /* Tras la última estación: las capas fijas se apagan (visibility, no solo
     opacity: los CTA del copy tienen pointer-events:auto) y dejan ver lo que
     sigue en el documento. El topbar y el cielo se quedan. */
  ...
  @media (prefers-reduced-motion:reduce){
    html[data-scroll-world]{scroll-snap-type:none;}
